...
pnpm install
pnpm typecheck && pnpm test        # 55/55
...
jobs:
  ci: pnpm install → typecheck → test (55/55) → demo → doctor
...
```
behavior-os/        # config, workflows (19), missions, runtime (evidence COMPLETED), state/control-plane.json, dnas
